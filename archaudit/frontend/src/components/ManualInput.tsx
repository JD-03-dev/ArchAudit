...
import { Play, ClipboardList, Info, HelpCircle, Copy, Check, Terminal, AlertCircle } from 'lucide-react';
import axios from 'axios';
import { cn, API_URL } from '../lib/utils';

export function ManualInput() {
  const { setStatus, setError, setResult, status, framework, error } = useAppStore();
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Failed to analyze structure');
      setStatus('error');
    }
  };
    ...
    <div className="space-y-4">
      ...
      {showHelp && (
        <div className="bg-slate-900/80 border border-slate-700/50 rounded-xl p-4 text-sm text-slate-300 space-y-3 animate-in fade-in slide-in-from-top-2">
          <p>Run these commands in your project root to copy the tree to your clipboard (or save to a file):</p>
          <div className="space-y-2">
            <div className="flex items-center justify-between bg-slate-950 p-2 rounded-lg border border-slate-800">
              ...
              <button onClick={() => copyCmd(macCmd, 'mac')} className="text-slate-500 hover:text-white p-1">
                {copiedMac ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} />}
              </button>
            </div>
            <div className="flex items-center justify-between bg-slate-950 p-2 rounded-lg border border-slate-800">
              <div className="flex items-center gap-2 font-mono text-xs overflow-x-auto text-emerald-400">
              ...
              <button onClick={() => copyCmd(winCmd, 'win')} className="text-slate-500 hover:text-white p-1">
                {copiedWin ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} />}
              </button>
            </div>
          </div>
        </div>
