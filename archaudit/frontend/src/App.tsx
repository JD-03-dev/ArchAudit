...
function App() {
  ...
  const [serverState, setServerState] = React.useState<'idle' | 'waking' | 'ready'>('idle');
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <button 
              ...
              className={cn(
                ...
                serverState === 'idle' ? "bg-slate-800 border-slate-700 text-slate-300 hover:text-white" :
                serverState === 'waking' ? "bg-amber-500/10 border-amber-500/20 text-amber-400" :
                "bg-emerald-500/10 border-emerald-500/20 text-emerald-400 cursor-default"
              )}
            >
              {serverState === 'idle' && <Power size={14} />}
              {serverState === 'waking' && <div className="w-3.5 h-3.5 border-2 border-amber-400/30 border-t-amber-400 rounded-full animate-spin" />}
              {serverState === 'ready' && <Server size={14} />}
              <span className="hidden sm:inline">
                {serverState === 'idle' ? 'Wake Server' : serverState === 'waking' ? 'Waking Server...' : 'Server Ready'}
              </span>
            </button>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                      <div className="max-h-60 overflow-y-auto py-1">
                        {FRAMEWORKS.map((fw) => (
                          <button
                            key={fw}
                            onClick={() => { setFramework(fw); setIsDropdownOpen(false); }}
                            className={cn(
                              "w-full text-left px-4 py-2.5 text-sm transition-colors",
                              framework === fw ? "bg-blue-500/10 text-blue-400" : "text-slate-300 hover:bg-slate-800/50 hover:text-white"
                            )}
                          >
                            {fw}
                          </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="p-6 border-b border-slate-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowScriptModal(false)} className="text-slate-400 hover:text-white transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="bg-amber-500/10 border border-amber-500/20 text-amber-300 p-4 rounded-xl flex gap-3 text-sm leading-relaxed">
                ...
                <div>
                  <p className="font-semibold mb-1">Warning: Review before executing</p>
                  <p className="opacity-90">This script was generated by AI and will create and move files on your machine. Always review the commands below and commit your current changes to git before running it.</p>
                </div>
              </div>

              <div className="space-y-2">
                <h4 className="font-medium text-slate-300 text-sm">How to use:</h4>
                <ol className="list-decimal list-inside text-sm text-slate-400 space-y-1">
                  ...
                  <li>Make it executable: <code className="bg-slate-800 text-emerald-400 px-1.5 py-0.5 rounded">chmod +x migrate.sh</code></li>
                  <li>Run it: <code className="bg-slate-800 text-emerald-400 px-1.5 py-0.5 rounded">./migrate.sh</code></li>
                </ol>
              </div>
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-end gap-3">
              <button onClick={() => setShowScriptModal(false)} className="px-4 py-2 rounded-xl text-slate-300 hover:text-white transition-colors font-medium text-sm">
                Cancel
              </button>
              <button onClick={handleDownloadScript} className="flex items-center gap-2 px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-sm transition-colors shadow-lg shadow-emerald-500/20">
                <Download size={16} />
                Download Script
              </button>
            </div>
